...
export default function BrandImportModal({ open, onClose, onImported }) {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      const { data } = await axios.post("/api/brands/import/validate", form, {
  ...
  const handleCommit = async (insertValidOnly = true) => {
    ...
    try {
      setCommitting(true);
      const { data } = await axios.post("/api/brands/import/commit", {
        token: res.token,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white w-[min(900px,95vw)] rounded-2xl shadow-xl overflow-hidden">
        <div className="px-6 py-4 border-b flex items-center justify-between">
          <h2 className="text-xl font-semibold">Import Brands (CSV)</h2>
          ...
          {!res && (
            <div className="space-y-3">
              <label className="block text-sm font-medium">
                Choose CSV file <span className="text-gray-500">(header: name[, image])</span>
              </label>
              ...
              <div className="text-sm text-gray-600">
                Need a sample?{" "}
                <a href="/api/brands/import/template" className="text-blue-600 underline">
                  Download CSV template
                </a>
              </div>
              <p className="text-xs text-gray-500">
                Tip: If you include an <code>image</code> column with HTTP/HTTPS URLs,
                the server will download and store them under <code>/storage/brands/imports</code>.
              </p>
            </div>
          )}
                  ...
                  <div className="max-h-64 overflow-auto border rounded">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 sticky top-0">
                        <tr><Th>Row#</Th><Th>Name</Th><Th>Image</Th><Th>Errors</Th></tr>
                      </thead>
                      <tbody>
                        {res.invalid_samples.map((r, i)=>(
                          <tr key={i} className="odd:bg-white even:bg-gray-50">
                            <Td>{r.row}</Td>
                            <Td>{r.data?.name}</Td>
                            <Td className="break-all">{r.data?.image}</Td>
                            <Td>
              ...
              {res.valid_samples?.length > 0 && (
                <div>
                  ...
                  <div className="max-h-48 overflow-auto border rounded">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 sticky top-0">
                        <tr><Th>Row#</Th><Th>Name</Th><Th>Image</Th></tr>
                      </thead>
                      <tbody>
        ...
        <div className="px-6 py-4 border-t flex items-center justify-between">
          {!res ? (
            <>
              <div className="text-sm text-gray-600">CSV header: <code>name[,image]</code></div>
