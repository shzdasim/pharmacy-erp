            ...
            <div className="flex items-center gap-2">
              ...
              <Guard when={can.create}>
                <Link
                  ...
                  className={`h-10 min-w-[160px] inline-flex items-center justify-center gap-2 rounded-xl px-3 ${tintBlue}`}
                >
        ...
        <GlassToolbar className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="md:col-span-2 flex items-center justify-between gap-3">
            <div className="text-sm text-gray-700">
              {loading ? "Loading…" : (
                <>Showing <strong>{filtered.length === 0 ? 0 : start + 1}-{Math.min(filtered.length, start + pageSize)}</strong> of <strong>{rows.length}</strong>{filtered.length !== rows.length && <> (filtered: <strong>{filtered.length}</strong>)</>}</>
              )}
            </div>
          ...
          <table className="w-full text-sm text-gray-900">
            <thead className="sticky top-0 bg-white/90 backdrop-blur-sm z-10 border-b border-gray-200/70">
              <tr className="text-left">
                ...
                <th className="px-3 py-2 font-medium text-right">Total Worth</th>
            ...
            <tbody>
              ...
              {paged.map((r, idx) => (
                <tr key={r.id} className="transition-colors odd:bg-white/90 even:bg-white/70 hover:bg-blue-50">
                  <td className="px-3 py-2">{start + idx + 1}</td>
                  <td className="px-3 py-2">{r.posted_number}</td>
                  <td className="px-3 py-2">{r.posted_date}</td>
                  <td className="px-3 py-2">{r.note}</td>
                  <td className="px-3 py-2 text-right">{r.items_count ?? r.items?.length ?? 0}</td>
                  <td className="px-3 py-2 text-right">{Number(r.total_worth || 0).toLocaleString()}</td>
                  <td className="px-3 py-2">
                    <div className="flex flex-wrap gap-2 justify-center">
                      ...
                      <Link
                        ...
                        className={`h-9 min-w-[100px] inline-flex items-center justify-center gap-1 rounded-xl px-3 ${tintGlass}`}
                        title="View"
                      >
