...
export default function CostOfSaleReport() {
  ...
  // --- Permissions as tri-state (true | false | null)
  const perms = usePermissions(); // call unconditionally
  ...
  return (
    <div className="p-4">
      ...
      {canView === null && (
        <div className="p-3 text-sm text-gray-600 bg-white rounded border border-gray-200">
          Checking permissions…
        </div>
      )}

      {canView === false && (
        <div className="p-3 text-sm text-gray-700 bg-yellow-50 border border-yellow-200 rounded">
          Permission denied.
        </div>
      )}
            ...
            <table className="min-w-[1100px] w-full text-sm">
              ...
              <tbody>
                ...
                {computed.withDerived.map((r, idx) => (
                  <tr key={r.sale_date + "_" + idx} className={idx % 2 ? "bg-white" : "bg-gray-50/50"}>
                    ...
                    <td className="px-3 py-2 text-right font-medium">{fmtCurrency(r.gp_amount)}</td>
                    <td className="px-3 py-2 text-right">{fmtPct(r.gp_pct)}</td>
                  </tr>
                ))}
              </tbody>
