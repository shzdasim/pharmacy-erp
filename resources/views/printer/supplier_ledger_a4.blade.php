...
<head>
  <meta charset="utf-8">
  <title>Supplier Ledger (A4)</title>
  <style>
...
<body onload="window.print()">
    ...
    <table>
      <thead>
        <tr>
          ...
          <th class="text-right">Payment (Debit)</th>
          <th>Payment Ref</th>
      ...
      <tbody>
        @forelse($rows as $r)
          <tr>
            ...
            <td class="text-right">{{ nf($r->total_paid) }}</td>
            <td class="text-right">{{ nf($r->debited_amount) }}</td>
            <td>{{ $r->payment_ref }}</td>
  ...
  <div class="mt-12" style="max-width: 500px;">
    <table>
      ...
      <tr><th>Paid on Invoice</th><td class="text-right">{{ nf($summary['paid_on_invoice']) }}</td></tr>
      <tr><th>Payments (Debited)</th><td class="text-right">{{ nf($summary['payments_debited']) }}</td></tr>
